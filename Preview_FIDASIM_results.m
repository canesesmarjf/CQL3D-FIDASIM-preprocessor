...
switch scenario
    ...
    case 99 % NEEDS to be fixed!!
        ...
        output_dir = fidasim_files_dir;
        cql3d_files_dir = "../run_archive/2024_09_05/cql3d_data/";
        plasma_file_name = "WHAM2expander_NB100_nitr40npz2_iter0_sh02_iy300jx300lz120_i11.nc";        
end
%% Plasma:
% Get data from files:
plasma.dene = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/plasma/dene');
plasma.te = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/plasma/te');
plasma.ti = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/plasma/ti');
plasma.mask = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/plasma/mask');
plasma.r2d = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/plasma/r2d');
plasma.z2d = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/plasma/z2d');
...
%% Fields:
% Get data from files:
info = h5info(fidasim_files_dir + runid + "_equilibrium.h5",'/fields');
fields.bz = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/fields/bz');
fields.bt = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/fields/bt');
fields.br = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/fields/br');
fields.mask = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/fields/mask');
fields.r2d = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/fields/r2d');
fields.z2d = h5read(fidasim_files_dir + runid + "_equilibrium.h5",'/fields/z2d');
...
%% Geometry:
info = h5info(fidasim_files_dir + runid + "_geometry.h5",'/nbi');
...
uvw_src  = h5read(fidasim_files_dir + runid + "_geometry.h5",'/nbi/src');

% NBI direction vector in machine coordinats:
uvw_axis = h5read(fidasim_files_dir + runid + "_geometry.h5",'/nbi/axis');

% Aperture distance along axis:
adist    = h5read(fidasim_files_dir + runid + "_geometry.h5",'/nbi/adist');

%% input.dat
file_path = fidasim_files_dir +  runid + "_inputs.dat";
...
if ~strcmpi(plasma_file_name,"none")
    file_name = cql3d_files_dir + plasma_file_name;    
...
% Get data from files:
info = h5info(fidasim_files_dir + runid + "_birth.h5");
n_birth = h5read(fidasim_files_dir + runid + "_birth.h5",'/n_birth');

% Ion birth positions in r-z-phi cylindrical coordinates:
birth.ri = h5read(fidasim_files_dir + runid + "_birth.h5",'/ri');

% Ion birth velocity vectors in r-z-phi cyclindrical coords:
birth.vi = h5read(fidasim_files_dir + runid + "_birth.h5",'/vi');

% Get grid data:
% 'X value of cell center in machine coordinates: x_grid(x,y,z)'
birth.U_grid = h5read(fidasim_files_dir + runid + "_birth.h5",'/grid/x');
birth.V_grid = h5read(fidasim_files_dir + runid + "_birth.h5",'/grid/y');
birth.W_grid = h5read(fidasim_files_dir + runid + "_birth.h5",'/grid/z');
...
birth.dens = h5read(fidasim_files_dir + runid + "_birth.h5",'/dens');
...
birth.weight = h5read(fidasim_files_dir + runid + "_birth.h5",'/weight');

% Get energy of each birth particle in [keV per ion]:
birth.energy = h5read(fidasim_files_dir + runid + "_birth.h5",'/energy');
...
% Check if file exits:
file_name = fidasim_files_dir + runid + "_sink.h5";
if exist(file_name, 'file') == 2
    ...
    sink.dens = h5read(fidasim_files_dir + runid + "_sink.h5",'/dens');
...
%% Neutral densities:
info = h5info(fidasim_files_dir + runid + "_neutrals.h5");
disp(" ")
...
denn_full_n = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/full/dens');
denn_half_n = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/half/dens');
denn_third_n = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/third/dens');

grid_x = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/grid/x_grid');
grid_y = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/grid/y_grid');
grid_z = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/grid/z_grid');
...
try
    denn_dcx_n = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/dcx/dens');
    denn_halo_n = h5read(fidasim_files_dir + runid + "_neutrals.h5",'/halo/dens');
