...
for ii = 1:size(beam_grid.vertices,1)
    uvw_vertices(ii,:) = beam_grid.origin + transpose(beam_grid.basis*transpose(beam_grid.vertices(ii,:)));
end
...
input_dir_nc = "../Step_6_WHAM_FIDASIM_cases_Yuri/Step_1_input/";
plasma_file_name = "WHAM2expander_NB100_nitr40npz2_iter0_sh09_iy300jx300lz120_yup5.nc";
file_name = input_dir_nc + plasma_file_name;
...
if exist(file_name, 'file') == 2
    ...
    sink.dens = h5read(input_dir + runid + "_birth.h5",'/dens');
    ...
    title('Ion sink profile $f_+f_B\langle{\sigma v_{rel}}\rangle_\phi^{CX}$','Interpreter','latex','FontSize',16);
    set(gca,'ZScale','lin')
    colormap(flipud(hot))
...
plot_heat_map_3d(h3d, theta_bins, z_bins, cx_energy_flux_wall, geom)
caxis([0,0.1])
